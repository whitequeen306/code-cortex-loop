...
**一条命令，七位领域专家接力体检，并自我进化。** 面向 AI 编码工具的「写完代码后」流水线：**正确性 → 安全 → 测试 → 错误处理 → 性能 → 精简 → 清理** —— 由 **Orchestrator 指挥 + 7 个 Task 子 agent 串行协作**（每人只负责本领域），配套健康分、handoff 接力、双语 Playbook、趋势看板、基线棘轮与 CI 集成。
...
输入 `/cortexloop`，**Orchestrator 按固定顺序启动 7 位领域专家**（每位专家 = 一次 mandatory Task），汇总 handoff 与健康分后，按你选择的模式产出：

- **Report 模式** —— 写出 `docs/cortexloop/*.md` + `.cortexloop/handoff/*.json` + `report.json` + **HTML 看板**，停下等你确认
- **Direct 模式** —— 分组增量修复、逐组跑测试、**复验时重跑七专家管道**，给出修复前后得分与趋势，并**自动反思沉淀经验**
- **CI 模式** —— 机器可读报告 + 退出码 + 可选 PR 评论

> Orchestrator **禁止** inline 分析代码；所有 pass 分析必须由对应 Task 子 agent 完成。详见 [七专家串行协作](#七专家串行协作)。
...
1. 运行 `install.ps1 -Tool cursor`（或 `install.sh cursor`），文件装到 `~/.cursor/{commands,agents,passes,skills,rules,scripts}/`。
2. 重启 Cursor。
...
1. 运行 `install.ps1 -Tool claude`（或 `install.sh claude`），文件装到 `~/.claude/{commands,agents,passes,skills,scripts}/`，并把 `AGENTS.md` 拷为 `~/.claude/AGENTS.cortexloop.md` 供参考。
...
1. 运行 `install.ps1 -Tool qoder`（或 `install.sh qoder`），文件装到 `~/.qoder/{commands,agents,passes,skills,rules,scripts}/`。
...
1. 运行 `install.ps1 -Tool opencode`（或 `install.sh opencode`），文件装到 `~/.config/opencode/{commands,agents,passes,skills,rules,scripts}/`。
2. 重启 OpenCode。
...
| `/cortexloop-quick` | 专家 1+2+4 串行（审查 + 安全 + 错误处理），近期改动，High+ |
| `/cortexloop-deep` | 全部 7 专家串行、整库扫描、强制基准测试 |
| `/cortexloop-security` | 专家 2+4+7 串行（安全 + 错误处理 + 依赖清理） |
...
```mermaid
flowchart LR
  pre["Step 0.5：查 verified 层"] --> pipe["七专家串行分析"]
  pipe --> fix["Direct 修复 + 测试"]
  fix --> reflect["反思 skill"]
  reflect --> record["record + playbook-zh"]
...
v2.2+ 将分析阶段从「一个会话身兼七职」改为 **1 指挥 + 7 领域专家接力**：每位专家通过 **Task 子 agent** 独立运行，只分析本领域，并将摘要写入 handoff 供下游专家阅读。

### 角色分工

| 角色 | 做什么 | 禁止做什么 |
|------|--------|------------|
| **Orchestrator**（主会话跑 `/cortexloop`） | 引导、定 scope、按序 launch Task、收集 handoff、聚合打分、Direct 应用 | 自己读代码做 pass 分析 |
| **领域专家**（每 pass 一个 Task） | 读本领域 + 上游 handoff，写类别报告 + handoff JSON | 分析其它 category、调用其它 agent |

### 固定顺序（enabled pass 按此接力）
...
Preset（如 `/cortexloop-quick`）只跑子集，但**相对顺序不变** —— quick 跑步骤 1、2、4。

### Handoff 协作

每个专家产出：

- **类别 Markdown** —— 人类可读 findings
- **Handoff JSON** —— 给下游专家的 1–3 句摘要 + scored findings + `deferToLaterPasses`（跨领域线索，不越界分析）

Schema：`schemas/pass-handoff.schema.json`。合约详情：`passes/README.md`（`passes/01-correctness.md` … `07-cleanup.md`）。

### 深度验证（Breadth → Depth）

每位专家先做 **breadth 扫描**（agent persona），再经 **depth skill** 确认 Evidence + Confidence 后才计入 findings。低置信项进入 `openQuestions`，不灌水。

**分析串行（七专家 Task 接力，只读），修复串行（每组之间跑测试）。**

---

## 7 路分析（专家 ↔ 深度 skill）

| 步 | 领域 | Agent（Task） | Depth skill |
|----|------|---------------|-------------|
| 1 | 正确性 & 架构 | `code-reviewer` | `code-review`, `edge-case-and-state-analysis` |
| 2 | 安全 | `security-auditor` | `security-review` + `security-hardening` rule |
| 3 | 测试策略 | `test-engineer` | `test-strategy`, `edge-case-and-state-analysis` |
| 4 | 错误处理 | `silent-failure-hunter` | `error-handling`, `edge-case-and-state-analysis` |
| 5 | 性能 | `performance-analyst` | `performance-optimization` |
| 6 | 简洁性 | `code-simplifier` | `simplify` |
| 7 | 清理 | `cleanup-curator` | `dead-code-and-deps` |

顺序常量（脚本/测试单源）：`scripts/lib/shared.mjs` → `PASS_PIPELINE`。
...
├── schemas/            # report、config、pass-handoff JSON schema
├── examples/           # demo 应用 + 演示
