...
面向 AI 编码工具的「写完代码后」审查 harness：先做轻量预检，按变更风险推荐 **Lite / Standard / Full**，再调度对应专家。产出健康分、HTML 报告与 CI 门禁；学习记忆、基线棘轮等高级项见后文。
...
**Lite 路径：** 输入 `/cortexloop-lite` → 选 **Report** → 跑完打开 [demo 看板](examples/demo-app/docs/cortexloop/report.html) 查看报告格式。不涉及 scope-map、Playbook、基线——后台仍会做必要落盘，你无需配置。
...
**⑤ 自进化，供下次使用**  
Direct 且复验通过后，工具会把**可复用的修复经验**写进项目内的 Playbook。下次再跑 `/cortexloop`，会**先翻翻以前验证过的套路**，再重新建索引、做 MAP、按档位审查——形成项目级修复经验的积累。给人看的进化笔记追加在同一份日志里，不会每次清空。
...
| **健康分 0–100** | 七维打分 + 总分；Direct 模式复验后重算分数 |
...
CodeCortexLoop 不是 SaaS，是 harness + CI/安装零额外依赖的 Node 脚本。
...
## 案例：LianYu-PC（Report 产物）

在 **Vue 3 + Spring Boot 全栈项目**上跑 `/cortexloop-full` 的 **Report 模式**（2026-06-22，整库扫描）的产物；完整 81 条明细见标准看板。
...
**代表性发现：** 验证码表达式泄露 · SSE 错误仍持久化 · 前端轮询静默失败 · 认证/SSE 核心路径零测试

| 链接 | 说明 |
|------|------|
| [showcase.html](examples/lianyu-pc/docs/cortexloop/showcase.html) | **Report → Direct 对比看板** |
