...
Sequential order: see `passes/README.md`. **Full Task**: Cursor, Claude Code, OpenCode (`full`). **Native Agent**: Qoder (`native`). **Partial**: Trae SOLO, Codex spawn (`partial`). Agent names: `PASS_AGENT_NAMES` in `scripts/lib/shared.mjs` (aliases: `OPENCODE_AGENT_NAMES`, etc.).
